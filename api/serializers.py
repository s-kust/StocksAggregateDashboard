...
class IndustriesSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = Industries
        fields = ('sector', 'industry', 'avg_margin', 'avg_leverage', 'avg_dividend_yield', 'avg_assets_turnover', 'companies')

class TickersSerializer(serializers.ModelSerializer):
    class Meta:
        model = Tickers
        fields = ('ticker', 'shortname', 'sector', 'industry', 'beta', 'forwardpe', 'margin', 'leverage', 'liquidity', 'dividendyield', 'assetsturnover')
        
class YearlySerializer(serializers.ModelSerializer):
    class Meta:
        model = Resultsyearly
        fields = ('enddate', 'margin', 'leverage', 'liquidity', 'cfpositive', 'nostockissuance', 'cfaboveincome')        
